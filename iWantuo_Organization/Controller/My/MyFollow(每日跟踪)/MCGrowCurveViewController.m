...
- (void)serverApi_FinishedSuccessed:(APIRequest *)api result:(APIResult *)sr {
    ...
    if (sr.status == 0) {
        ...
        if (api == self.api) {
            ...
            NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"您的孩子打败了同级%@%%的同学", self.overRun]];
            [str addAttribute:NSForegroundColorAttributeName value:kNavigationColor range:NSMakeRange(9,self.overRun.length + 1)];
            [str addAttribute:NSFontAttributeName value:[UIFont fontWithName:@"Helvetica-Bold" size:20] range:NSMakeRange(9,self.overRun.length + 1)];
