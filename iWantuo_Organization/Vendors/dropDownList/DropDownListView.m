...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate superView:(UIView *)superView
{
    ...
    if (self) {
        ...
        self.downView = superView;
        //初始化默认显示view
        CGFloat sectionWidth = (1.0*(frame.size.width)/sectionNum);
        for (int i = 0; i <sectionNum; i++) {
            UIButton *sectionBtn = [[UIButton alloc] initWithFrame:CGRectMake(sectionWidth*i, 0, sectionWidth, frame.size.height)];
            sectionBtn.tag = SECTION_BTN_TAG_BEGIN + i;
            [sectionBtn addTarget:self action:@selector(sectionBtnTouchTest:) forControlEvents:UIControlEventTouchUpInside];
            NSString *sectionBtnTitle = @"--";
            if ([self.dropDownDataSource respondsToSelector:@selector(titleInSection:index:)]) {
                sectionBtnTitle = [self.dropDownDataSource titleInSection:i index:[self.dropDownDataSource defaultShowSection:i]];
            }
...
-(void)sectionBtnTouchTest:(UIButton *)btn
{
    FSDropDownMenu *menu = (FSDropDownMenu*)[self.downView viewWithTag:1001];
    if (menu.show) {
        [menu menuTapped];
    }
...
{
    if (!self.mTableView) {
        self.mTableBaseView = [[UIView alloc] initWithFrame:CGRectMake(self.mSuperView.frame.origin.x, self.mSuperView.frame.origin.y + self.frame.size.height +64, self.mSuperView.frame.size.width, self.mSuperView.frame.size.height - self.frame.origin.y - self.frame.size.height)];
    ...
    rect.size.width = self.mSuperView.frame.size.width;
    rect.size.height = 0;
    self.mTableView.frame = rect;
